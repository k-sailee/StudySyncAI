...
export default function MindMap(): JSX.Element {
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-br from-background via-background to-background/95">
      {/* Simplified Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-blue-500/50 bg-gradient-to-r from-slate-900 via-blue-900/30 to-slate-900 shadow-lg">
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-extrabold text-blue-300 drop-shadow-lg">🧠 Mind Map</h2>
        ...
        <div className="flex gap-3">
          <button 
            onClick={handleNew}
            className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition-all transform hover:scale-105 flex items-center gap-1.5"
          ...
          <button 
            ...
            className="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition-all transform hover:scale-105 flex items-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
          ...
          <button 
            ...
            className="bg-cyan-600 hover:bg-cyan-500 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition-all transform hover:scale-105 flex items-center gap-1.5"
          ...
          <button 
            ...
            className="bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition-all transform hover:scale-105 flex items-center gap-1.5"
          ...
          <button 
            onClick={clearCanvas}
            className="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-md hover:shadow-lg transition-all transform hover:scale-105"
            title="Clear canvas"
          >
            Clear
          </button>
      ...
      <div className="px-6 py-3 border-b border-border/10 bg-transparent">
        <button
          onClick={() => navigate('/dashboard')}
          className="bg-slate-700 hover:bg-slate-600 text-white px-3 py-2 rounded-md text-sm font-semibold shadow-sm hover:shadow-md transition-colors"
        >
          Back
        </button>
      </div>
      ...
      {showCreateDialog && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-background border border-border/20 rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
            <h3 className="text-lg font-bold text-slate-900 mb-2">Create New Mind Map</h3>
            <p className="text-xs text-slate-700 mb-4">Give your mind map a name to save it</p>
            <input
              type="text"
              value={mapTitle}
              onChange={(e) => setMapTitle(e.target.value)}
              placeholder="e.g. Physics Notes, Project Planning..."
              className="w-full px-4 py-2 bg-white border border-border/30 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-300 mb-4"
              autoFocus
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow-md hover:shadow-lg transition-all"
              >
                Create & Save
              </button>
              <button
                ...
                className="flex-1 bg-gray-200 text-slate-900 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-300 transition-colors"
              >
                Cancel
              </button>
            </div>
      ...
      {showOpenDialog && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-background border border-border/20 rounded-xl p-6 max-w-2xl w-full mx-4 shadow-2xl max-h-[80vh] overflow-hidden flex flex-col">
            <h3 className="text-lg font-bold text-slate-900 mb-4">Open Mind Map</h3>
            
            <div className="flex-1 overflow-y-auto space-y-2 mb-4">
              {savedMaps.length === 0 ? (
                <p className="text-slate-600 text-center py-8">No saved mind maps yet. Create one first!</p>
              ) : (
                savedMaps.map((map) => (
                  <div
                    key={map._id}
                  ...
                  >
                    <div className="flex-1">
                      <h4 className="text-slate-900 font-semibold">{map.title}</h4>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow-md hover:shadow-lg"
                      >
                        Open
                      </button>
                      <button
                        onClick={() => handleDelete(map._id, map.title)}
                        className="text-red-600 px-3 py-2 rounded-lg hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100"
            ...
            <button
              ...
              className="bg-gray-200 text-slate-900 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-300 w-full"
            >
              Close
            </button>
          </div>
